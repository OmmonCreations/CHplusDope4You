...
namespace TMPro
{
    ...
    public class TextPro3dOnACurve : MonoBehaviour
    {
        ...
        [SerializeField] private float _curveScale = 1.0f;

        private Vector3[] _vertices;
        private Matrix4x4 _matrix;

        private float _oldCurveScale;
        ...
        private void Start()
        {
            ...
            _curveScale *= 10;
            _oldCurveScale = _curveScale;
            _oldCurve = CopyAnimationCurve(_vertexCurve);
        }

        private void LateUpdate()
        {
            if (!_mTextComponent.havePropertiesChanged && Mathf.Abs(_oldCurveScale - _curveScale) <= 0 &&
                Mathf.Abs(_oldCurve.keys[1].value - _vertexCurve.keys[1].value) <= 0)
            {
                return;
            }
