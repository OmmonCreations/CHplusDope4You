Shader "Roystan/Grass"
{
	...
	void geo(triangle vertexOutput IN[3], inout TriangleStream<geometryOutput> triStream)
	{
		...
		for (int i = 0; i < BLADE_SEGMENTS; i++)
		{
			...
			float segmentHeight = height * t * scale;
			float segmentWidth = width * (1 - t) * scale;
			float segmentForward = pow(t, _BladeCurve) * forward * scale;
		...
		triStream.Append(GenerateGrassVertex(pos, 0, height * scale, forward * scale, float2(0.5, 1), transformationMatrix));
	}
	ENDCG
